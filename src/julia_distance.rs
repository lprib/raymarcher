...
pub fn draw_ray2d(frame: &mut [u32], mouse_x: f64, mouse_y: f64) {
    ...
    draw_point(frame, mouse_x, mouse_y);
    //march rays

    let start_point = Vec3::from((0, super::WIDTH as i32, 0));
    ...
    loop {
        ...
        let radius = dist_estimate(zx, zy) * 20.0;
        t += radius;
        iters += 1;
        current_point = start_point + t * dir;
        draw_point(frame, current_point.x, current_point.y);
...
fn draw_point(frame: &mut [u32], x: f64, y: f64) {
    if x >= 0.0 && y >= 0.0 && x < super::WIDTH as f64 && y < super::HEIGHT as f64 {
        frame[y as usize * super::WIDTH + x as usize] = Vec3::from((1, 0, 0)).into();
    }
}
